...
from lawu.constants import *
from lawu.util.descriptor import method_descriptor
...
class EntityTopping(Topping):
  ...
  def act(aggregate, classloader, verbose=False):
    ...
    if entity_format in handlers:
      ...
      if verbose:
        print(f'Unknown entity format {entity_format}')
      return
      ...
      def on_invoke(self, ins, const, obj, args):
        if const.class_.name == listclass:
          ...
          if ins.mnemonic != "invokestatic":
            if len(
                args) == 2 and const.name_and_type.descriptor.value.startswith(
                    "(FF)"):
          ...
          if len(args) == 2:
            if desc.args[0].name == "java/lang/Class" and desc.args[
              ...
              if verbose:
                print("Unknown entity type builder creation method",
                      method_desc)
              cls = None
          elif len(args) == 1:
      ...
      def on_put_field(self, ins, const, obj, value):
        if isinstance(value, dict):
          ...
          if "class" not in value:
            ...
            value["class"] = sig[sig.index("<") + 2:sig.index(">") -
        ...
        if const.class_.name == listclass:
          if len(args) == 4:
            ...
            if old_name + ".name" in aggregate["language"]["entity"]:
              entity[name]["display_name"] = aggregate["language"]["entity"][
                  old_name + ".name"]
          elif len(args) == 3:
        ...
        if ins in ("ldc", "ldc_w"):
          ...
          if mode == "entities":
            ...
            if tmp["name"] + ".name" in aggregate["language"]["entity"]:
              tmp["display_name"] = aggregate["language"]["entity"][tmp["name"]
                                                                    + ".name"]
            entity[tmp["name"]] = tmp
          elif mode == "aliases":
            tmp["entity"] = stack[0]
  ...
  def compute_sizes(classloader, aggregate, entities):
    ...
    base_entity_cf = classloader[aggregate["entities"]["entity"]
  ...
  def abstract_entities(classloader, entities, verbose):
    entity_classes = {e["class"]: e["name"] for e in entities.values()}
      ...
      for name in subclass_names:
        if name in entities:
          ...
          if parent not in entity_classes:
            ...
            print("Unexpected non-abstract class for parent of %s: %s" %
                  (name, entity_classes[parent]))
          break
      else:
        if verbose:
          print("Failed to find abstract entity %s as a superclass of %s" %
    ...
    abstract_entity("insentient", "ender_dragon",
                    "EnderDragon")  # EntityLiving
